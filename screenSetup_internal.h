...
{
public:
                    Item(  ILI9341 *tft,int line) {item_tft=tft;_line=line;_state=StateNormal;}
    virtual         ~Item() {}
...
{
public:
                    SimpleItem(  ILI9341 *tft, int line,const char *name ) : Item(tft,line) {_name=name;}
    virtual         ~SimpleItem() {}    
    virtual void    run(int leftright) {};
...
{
public:
                TunableItem(ILI9341 *tft,int line,int *value, int mn, int mx, int inc, const char *name, const char *unit);                      
        void    drawItem();
        void    update();
